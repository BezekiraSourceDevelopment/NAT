...
public class FileMessages {
	...
	public static File file = new File("config/nexus/messages.conf");
	...
	public static void setup() {
		...
		try {
			...
			if (!file.exists()) {
				...
				messages.getNode("events", "join", "message").setValue("&6%player &7has joined.");
				
				messages.getNode("events", "leave", "enable").setValue(true);
				messages.getNode("events", "leave", "message").setValue("&6%player &7has left.");
				
				messages.getNode("events", "firstjoin", "enable").setValue(true);
				messages.getNode("events", "firstjoin", "message").setValue("&6%player &7has joined for the first time!");
				messages.getNode("events", "firstjoin", "uniqueplayers", "show").setValue(true);
				messages.getNode("events", "firstjoin", "uniqueplayers", "message").setValue("&6%players &7unique players already joined.");
