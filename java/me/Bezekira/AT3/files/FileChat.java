...
public class FileChat {
	...
	public static File file = new File("config/nexus/chat.conf");
		...
		try {
			...
			if(chat.getNode("channels").hasMapChildren()) {
				for(Entry<Object, ? extends ConfigurationNode> e : chat.getNode("channels").getChildrenMap().entrySet()) {
					...
					NexusChannel c = new NexusChannel(channel, trigger, name, prefix, suffix, format, range);
					NexusChannels.add(channel, c);
