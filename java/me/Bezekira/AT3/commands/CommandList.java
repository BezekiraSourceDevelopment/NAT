...
public class CommandList implements CommandCallable {
	...
	public CommandResult process(CommandSource sender, String arguments) throws CommandException {
		...
		if(!PermissionsUtils.has(sender, "nexus.list")) { sender.sendMessage(Text.builder("You do not have permissions!").color(TextColors.RED).build()); return CommandResult.success(); }

		int online = Controller.getServer().getOnlinePlayers().size();
		int max = Controller.getServer().getMaxPlayers();

		sender.sendMessage(Text.of(TextColors.GRAY, "There are currently ", TextColors.YELLOW, online, TextColors.GRAY, "/", TextColors.YELLOW, max, TextColors.GRAY, " players online."));
