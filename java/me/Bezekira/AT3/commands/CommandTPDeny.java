...
public class CommandTPDeny implements CommandCallable {
	...
	public CommandResult process(CommandSource sender, String arguments) throws CommandException {
		...
		if(!PermissionsUtils.has(sender, "nexus.tpdeny")) { sender.sendMessage(Text.builder("You do not have permissions!").color(TextColors.RED).build()); return CommandResult.success(); }

		if(arguments.equalsIgnoreCase("")) { sender.sendMessage(Text.of(TextColors.GOLD, "Usage: ", TextColors.GRAY, "/tpdeny <player>")); return CommandResult.success(); }

		if(args.length < 1 || args.length > 1) { sender.sendMessage(Text.of(TextColors.GOLD, "Usage: ", TextColors.GRAY, "/tpdeny <player>")); return CommandResult.success(); }
		...
		sender.sendMessage(Text.of(TextColors.GOLD, target.getName(), "'s ", TextColors.GRAY, "teleport request has been rejected."));
	...
	private final Text usage = Text.builder("Usage: /tpdeny <player>").color(TextColors.GOLD).build();
	private final Text help = Text.builder("Help: /tpdeny <player>").color(TextColors.GOLD).build();
	private final Text description = Text.builder("Nexus | TPDeny Command").color(TextColors.GOLD).build();
