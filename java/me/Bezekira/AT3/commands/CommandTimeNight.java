...
public class CommandTimeNight {
	...
	public CommandTimeNight(CommandSource sender, String[] args, Game game) {
		...
		if(!PermissionsUtils.has(sender, "nexus.time.night")) { sender.sendMessage(Text.builder("You do not have permissions!").color(TextColors.RED).build()); return; }

		if(args.length == 2) {

			if(args[1].equalsIgnoreCase("*")) {
				for(World world : Controller.getServer().getWorlds()) world.getProperties().setWorldTime(14000);
				ServerUtils.broadcast(Text.of(TextColors.GOLD, sender.getName(), TextColors.GRAY, " has changed the time to night."));
				return;
			}
			...
			ServerUtils.broadcast(Text.of(TextColors.GOLD, sender.getName(), TextColors.GRAY, " has changed the time to night on ", TextColors.GOLD, world.getName()));
