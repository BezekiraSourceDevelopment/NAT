...
public class CommandButcher implements CommandCallable {
	...
	public CommandResult process(CommandSource sender, String arguments) throws CommandException {
		...
		if(!PermissionsUtils.has(sender, "nexus.butcher")) { sender.sendMessage(Text.builder("You do not have permissions!").color(TextColors.RED).build()); return CommandResult.success(); }

		if(args.length < 1) { sender.sendMessage(Text.of(TextColors.GOLD, "Usage: ", TextColors.GRAY, "/butcher [radius]")); return CommandResult.success(); }
		...
		sender.sendMessage(Text.of(TextColors.GRAY, "Removed ", TextColors.GOLD, c, TextColors.GRAY, " entities."));

		return CommandResult.success();

	}

	private final Text usage = Text.builder("Usage: /butcher [radius]").color(TextColors.GOLD).build();
	private final Text help = Text.builder("Help: /butcher [radius]").color(TextColors.GOLD).build();
	private final Text description = Text.builder("Nexus | Butcher Command").color(TextColors.GOLD).build();
