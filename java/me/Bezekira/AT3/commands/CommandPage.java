...
public class CommandPage implements CommandCallable {
	...
	public CommandResult process(CommandSource sender, String arguments) throws CommandException {
		...
		if(pages.containsKey(page - 1)) {
			page_previous = Text.builder("Previous Page").color(TextColors.YELLOW).onHover(TextActions.showText(Text.of(TextColors.GRAY, "Go to previous site"))).onClick(TextActions.runCommand("/page " + String.valueOf(page - 1))).build();
		}
		if(pages.containsKey(page + 1)) {
			page_next = Text.builder("Next Page").color(TextColors.YELLOW).onHover(TextActions.showText(Text.of(TextColors.GRAY, "Go to next site"))).onClick(TextActions.runCommand("/page " + String.valueOf(page + 1))).build();
		}
		Text footer = Text.of();
	...
	private final Text usage = Text.builder("Usage: /page <page>").color(TextColors.GOLD).build();
	private final Text help = Text.builder("Help: /page <page>").color(TextColors.GOLD).build();
	private final Text description = Text.builder("Nexus | Page Command").color(TextColors.GOLD).build();
