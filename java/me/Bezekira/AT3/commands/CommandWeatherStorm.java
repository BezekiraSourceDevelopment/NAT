...
public class CommandWeatherStorm {
	...
	public CommandWeatherStorm(CommandSource sender, String[] args, Game game) {
		...
		if(!PermissionsUtils.has(sender, "nexus.weather.storm")) { sender.sendMessage(Text.builder("You do not have permissions!").color(TextColors.RED).build()); return; }

		if(args.length == 2) {

			if(args[1].equalsIgnoreCase("*")) {
				for(World world : Controller.getServer().getWorlds()) world.setWeather(Weathers.THUNDER_STORM);
				ServerUtils.broadcast(Text.of(TextColors.GOLD, sender.getName(), TextColors.GRAY, " has changed the weather to storm."));
				return;
			}
			...
			ServerUtils.broadcast(Text.of(TextColors.GOLD, sender.getName(), TextColors.GRAY, " has changed the weather to storm on ", TextColors.GOLD, world.getName()));
